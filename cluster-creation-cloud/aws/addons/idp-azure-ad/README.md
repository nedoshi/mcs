# Azure AD OIDC IdP (post-cluster addon)

**Standalone** — requires existing `cluster_id`. Path: `cluster-creation/aws/addons/idp-azure-ad/` (formerly `tf-rosa-idp-terraform-cloud-setup/`).
...
**Makefile:** Run `make help` for targets (`init`, `plan`, `apply`, `destroy`, `validate`). Use `TFVARS=file.tfvars` if needed.
