...
# Create TLS secret if it doesn't exist (required by Keycloak operator)
echo "Checking for TLS secret..."
if ! oc get secret my-tls-secret -n $NAMESPACE &> /dev/null; then
    echo "TLS secret not found. Creating self-signed certificate..."
    ...
    if command -v openssl &> /dev/null; then
        ...
        if [ -f "$TEMP_DIR/keycloak-tls.key" ] && [ -f "$TEMP_DIR/keycloak-tls.crt" ]; then
            ...
            rm -rf "$TEMP_DIR"
            echo "✅ TLS secret created with self-signed certificate"
        else
            echo "⚠️  Failed to generate certificate. Please create the secret manually."
            echo "   Run: oc create secret tls my-tls-secret --cert=tls.crt --key=tls.key -n $NAMESPACE"
        fi
    else
        echo "⚠️  openssl not available. Please create the TLS secret manually:"
        echo "   oc create secret tls my-tls-secret --cert=tls.crt --key=tls.key -n $NAMESPACE"
        echo "   Or generate a self-signed cert first with openssl"
    fi
else
